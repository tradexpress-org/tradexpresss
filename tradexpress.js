...
(function(window){
  ...
  function buildDebugSnapshot(){
    const instance = window.TradExpress || {};
    return {
      ready: debugState.ready,
      selector: debugState.selector,
      theme: debugState.theme,
      articleCount: debugState.articleCount,
      source: debugState.source,
      enableAimAsk: debugState.enableAimAsk,
      containerFound: debugState.containerFound,
      lastError: debugState.lastError,
      articlePreview: (instance.articles || []).slice(0, 5).map((article)=>({
        id: article.id || null,
  ...
  const TradExpress = {
    ...
    init: function(opts){
      ...
      if(!container){
        debugState.lastError = 'Container not found: ' + selector;
        console.warn('[TradExpress] Container not found:', selector);
        return;
      }
      this.container = container;
      this.theme = opts.theme || 'dark';
      this._aimAskRequested = opts.enableAimAsk !== false;
      this.enableTxBot = opts.enableTxBot !== false;
      this.enableAimAsk = this.enableTxBot && this._aimAskRequested;
      this.currentFilter = {q:'', tag:''};
      this.activeView = opts.activeView || 'chat';
      debugState.ready = true;
      debugState.selector = selector;
      debugState.theme = this.theme;
      debugState.enableAimAsk = this.enableAimAsk;
      debugState.txBotEnabled = this.enableTxBot;
      debugState.containerFound = true;
      debugState.source = opts.articlesUrl || (opts.articles && Array.isArray(opts.articles) ? 'inline' : 'default');

      // Load articles
      if(opts.articles && Array.isArray(opts.articles)){
        this.articles = opts.articles;
        debugState.articleCount = this.articles.length;
        this.render();
        ...
        fetch(opts.articlesUrl)
          ...
          .then(data=>{
            ...
            debugState.lastError = e.message || 'Failed to load articles';
            console.error('[TradExpress] Failed load', e);
          });
      } else {
      ...
      if(!document.getElementById('tradexpress-style')){
        ...
        style.textContent = `
          ...
          .tx-settings { display:flex; align-items:center; gap:8px; flex-wrap:wrap; margin-bottom:12px; }
          .tx-help { font-size:11px; opacity:0.7; color:#9FB2CB; }
          .tx-bot-toggle { padding:6px 12px; border-radius:999px; border:1px solid var(--border); background:rgba(255,255,255,0.08); color:white; font-size:12px; font-weight:700; cursor:pointer; }
          .tx-bot-toggle.active { background:linear-gradient(135deg,#2A6FFF,#00E5FF); color:white; }
          .tx-viewbar { display:flex; gap:8px; flex-wrap:wrap; margin:8px 0 12px; }
          .tx-viewchip { padding:7px 11px; border-radius:999px; border:1px solid var(--border); background:rgba(255,255,255,0.06); color:#DCE7FF; font-size:12px; cursor:pointer; }
          .tx-viewchip.active { background:linear-gradient(135deg, rgba(42,111,255,0.3), rgba(0,229,255,0.2)); color:white; border-color:rgba(0,229,255,0.45); }
          .tx-workspace { border:1px solid var(--border); border-radius:16px; padding:12px; background:rgba(255,255,255,0.04); margin-bottom:12px; }
          .tx-workspace-title { font-size:12px; font-weight:700; letter-spacing:0.12em; text-transform:uppercase; opacity:0.7; margin-bottom:8px; }
          .tx-premium-pill { display:inline-flex; align-items:center; gap:6px; padding:6px 10px; border-radius:999px; background:linear-gradient(135deg, rgba(42,111,255,0.28), rgba(0,229,255,0.18)); color:#EAF7FF; font-size:11px; font-weight:800; margin-bottom:8px; }
          .tx-workspace-row { display:flex; justify-content:space-between; align-items:center; padding:8px 10px; border-radius:10px; background:rgba(255,255,255,0.04); margin-top:6px; font-size:13px; }
          .tx-workspace-row strong { color:#7EE9FF; }
          .tx-action-btn { margin-top:8px; border:none; border-radius:999px; padding:7px 12px; cursor:pointer; font-size:12px; font-weight:700; background:linear-gradient(135deg,#2A6FFF,#00E5FF); color:white; }
          .tx-dialog-backdrop { position:fixed; inset:0; background:rgba(2,6,23,0.72); display:flex; align-items:center; justify-content:center; z-index:9999; padding:16px; }
          .tx-dialog { width:min(460px, 100%); background:linear-gradient(180deg, rgba(10,25,49,0.98), rgba(8,16,34,0.98)); border:1px solid rgba(255,255,255,0.12); border-radius:18px; padding:16px; box-shadow:0 18px 48px rgba(0,0,0,0.35); }
          .tx-dialog-title { font-size:16px; font-weight:800; margin-bottom:10px; }
          .tx-dialog-row { display:flex; justify-content:space-between; align-items:center; padding:10px 0; border-bottom:1px solid rgba(255,255,255,0.08); font-size:13px; }
          .tx-dialog-row:last-child { border-bottom:none; }
          .tx-link { color:#7EE9FF; text-decoration:none; font-weight:700; }
          .tx-close-btn { margin-top:12px; border:none; border-radius:999px; padding:8px 12px; cursor:pointer; font-weight:700; background:white; color:#0A1931; }
          /* Aim & Ask reuse */
    ...
    render: function(filter){
      ...
      const settings = el('div','tx-settings');
      const txBotToggle = el('button','tx-bot-toggle'+(this.enableTxBot?' active':''), this.enableTxBot ? 'TXBot: ON' : 'TXBot: OFF');
      txBotToggle.onclick = ()=> this.setTxBotEnabled(!this.enableTxBot);
      settings.appendChild(txBotToggle);
      settings.appendChild(el('div','tx-help', this.enableTxBot ? 'TXBot is active for this embed.' : 'TXBot is disabled. Turn it on to enable the assistant.'));
      c.appendChild(settings);

      const viewBar = el('div','tx-viewbar');
      ['chat','starred','editor'].forEach((view)=>{
        const chip = el('button','tx-viewchip'+(this.activeView===view?' active':''), view === 'chat' ? 'Chat' : view === 'starred' ? 'Starred' : 'Editor');
        chip.onclick = ()=>{ this.activeView=view; this.render(this.currentFilter || {q:'', tag:''}); };
        viewBar.appendChild(chip);
      });
      c.appendChild(viewBar);

      const workspace = el('div','tx-workspace');
      workspace.innerHTML = `
        <div class="tx-workspace-title">TXBot workspace</div>
        <div class="tx-premium-pill">★ Kenwell + TradExpress</div>
        <div class="tx-workspace-row"><span>Partner</span><strong>Digital trade partner</strong></div>
        <div class="tx-workspace-row"><span>Chat</span><strong>Unlimited access</strong></div>
        <div class="tx-workspace-row"><span>Starred</span><strong>Saved follow-ups</strong></div>
        <div class="tx-workspace-row"><span>Editor</span><strong>Draft support notes</strong></div>
        <button class="tx-action-btn" type="button">Open hello@tradexpress.co</button>
      `;
      workspace.querySelector('.tx-action-btn').onclick = ()=> this.openSupportDialog();
      c.appendChild(workspace);
      ...
      // Footer with contact
      const footer = el('div','', `<div style="margin-top:18px;text-align:center;font-size:11px;opacity:0.4">Powered by TradExpress.co IMPACT • ${this.articles.length} articles • Contact hello@tradexpress.co / hello@kailu.dev • TXBot ${this.enableTxBot ? 'enabled' : 'disabled'}</div>`);
    ...
    openSupportDialog: function(){
      const backdrop = el('div','tx-dialog-backdrop');
      const dialog = el('div','tx-dialog');
      dialog.innerHTML = `
        <div class="tx-dialog-title">TXBot support</div>
        <div class="tx-dialog-row"><span>Partner</span><strong>Kenwell + TradExpress</strong></div>
        <div class="tx-dialog-row"><span>Model</span><strong>Digital trade partner</strong></div>
        <div class="tx-dialog-row"><span>Access</span><strong>Unlimited for premium</strong></div>
        <div class="tx-dialog-row"><span>Starred</span><strong>Saved follow-ups</strong></div>
        <div class="tx-dialog-row"><span>Editor</span><strong>Human-ready drafts</strong></div>
        <div class="tx-dialog-row"><span>Email</span><a class="tx-link" href="mailto:hello@tradexpress.co">hello@tradexpress.co</a></div>
        <button class="tx-close-btn" type="button">Close</button>
      `;
      backdrop.appendChild(dialog);
      dialog.querySelector('.tx-close-btn').onclick = ()=> backdrop.remove();
      document.body.appendChild(backdrop);
    },
    setTxBotEnabled: function(enabled){
      this.enableTxBot = !!enabled;
      this.enableAimAsk = this.enableTxBot && this._aimAskRequested;
      debugState.txBotEnabled = this.enableTxBot;
      debugState.enableAimAsk = this.enableAimAsk;
      if(this.container){
        const existing = this.container.parentElement?.querySelector('.aim-btn');
        if(!this.enableTxBot){
          existing?.remove();
        } else if(this.enableAimAsk && window.TradExpressAim) {
          window.TradExpressAim.attach(this.container);
        }
        this.render(this.currentFilter || {q:'', tag:''});
      }
    },
    ...
    debugHealth: function(){
      ...
      return { ...snapshot, ok: snapshot.containerFound && snapshot.ready };
    }
  };
  ...
  const TradExpressAim = {
    attach: function(container){
      ...
      const btn = el('button','aim-btn','🤖 TXBot');
      btn.type='button';
      ...
      btn.addEventListener('click', (e)=>{
        ...
        btn.textContent = isAiming ? '❌ Exit TXBot' : '🤖 TXBot';
      ...
      document.addEventListener('click', (e)=>{
        ...
        if(current){
          ...
          isAiming=false; btn.classList.remove('active'); btn.textContent='🤖 TXBot'; document.body.classList.remove('crosshair-mode'); current.classList.remove('crosshair-target'); current=null;
        }
      });
    }
  };
  ...
  window.txbotDebug = {
    ...
    ping: function(){
      return { ok: true, message: 'TX bot debug ready' };
    }
  };
  ...
  window.addEventListener('message', (e)=>{
    ...
    if(e.data && e.data.type==='txbot:debug'){
      const payload = window.txbotDebug ? window.txbotDebug.snapshot() : { ok: false };
      if(e.source && typeof e.source.postMessage === 'function') {
        e.source.postMessage({ type: 'txbot:debug:response', payload }, '*');
      }
    }
  });
